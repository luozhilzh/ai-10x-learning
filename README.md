...
- **浏览器打开**：双击 `.html` 文件，确认能正常渲染、页面里没有 `{{...}}` 残留字样。
- **脚本校验**：跑 `python scripts/validate_card.py <你的卡.html>`，自动检查 ① 无残留占位符 ② 含 `<!DOCTYPE html>` ③ `<html>` / `</html>` 闭合 ④ 体积合理。通过打印 `[PASS]`，失败打印 `[FAIL]` 与具体原因（退出码 0/1，可接入 CI）。
